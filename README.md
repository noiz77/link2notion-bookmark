...
*   **🔖 书签模式：** 
    *   **适用场景：** 把网页作为带预览的 Bookmark 卡片存入 Notion。
    *   **单链接（默认）：** 自动取当前标签页的网址；可在"备注（选填）"框中书写内容（该内容将展示在书签卡片下方）。
    *   **封面图获取：** 开启开关后，会连同网页自身的 Open Graph 大图一起抓取，带来极佳的全幅视觉效果。
    *   **⚡ 批量开关：** 可批量粘贴 URL（一行一个）或点击"填充链接"把当前标签页加入暂存队列。封面开关同样可用——逐条按"能抓到 og:image 就加封面图块，抓不到就跳过"处理。

*   **📄 文章模式：** 
    *   **适用场景：** 将当前网页的正文清洗、排版后，保存为可读的 Notion 笔记（适合长文与博客类页面）。
    *   **特性：** 基于正文解析提取标题与段落；导入成功后可在一定条件下自动跳转到 Notion 中新建页面。若需要先限定范围，可在页面上框选正文再导入。
    *   **Database：** 写入 Database 时会正确识别目标类型，避免普通页面误判导致的写入失败；同名属性不会重复创建。

*   **🐦 推特（X）长文模式：** 
    *   **适用场景：** 一键保存并结构化重排整条 X/Twitter 长推文（Thread）或 X Article 长文页。
    *   **特性：** 在此模式下点击导入，助手会自动将作者、日期、推文文字及长文分段等解析排版，支持代码块等内容导入，直接转存为 Notion 阅读笔记。
    *   **智能分配：** 如果您的目标是一个普通的 Notion Page，信息会精美地排版在顶部导语；如果您的目标是 Database，它会自动新建或复用名为 `Author`、`Date`、`Tags`、`URL` 的属性并填入数据。
